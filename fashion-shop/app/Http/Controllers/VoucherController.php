...
class VoucherController extends Controller
{
    ...
    private function validateCreateVoucherData(Request $request): array
    {
        return $request->validate([
            ...
            'max_discount' => ['nullable', 'numeric', 'min:0', 'max:9999999999.99'],
            'usage_limit' => ['nullable', 'integer', 'min:1'],
            'start_date' => ['required', 'date'],
            'end_date' => ['required', 'date', 'after_or_equal:start_date'],
    ...
    private function mapVoucherPayload(array $validated): array
    {
        return [
            ...
            'category_id' => $validated['category'] === 'category' ? ($validated['category_id'] ?? null) : null,
            'collection_id' => $validated['category'] === 'collection' ? ($validated['collection_id'] ?? null) : null,
            'product_id' => $validated['category'] === 'product' ? ($validated['product_id'] ?? null) : null,
            'discount_type' => $validated['discount_type'],
            'discount_value' => $validated['discount_value'],
            'min_order_value' => $validated['min_order_value'],
            'max_discount' => $validated['max_discount'] ?? 0,
            'usage_limit' => $validated['usage_limit'] ?? null,
            'start_date' => $validated['start_date'],
            'end_date' => $validated['end_date'],
